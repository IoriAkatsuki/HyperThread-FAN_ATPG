...
// **************************************************************************
void Simulator::parallelFaultFaultSimWithAllPattern(PatternProcessor *pPatternCollector, FaultListExtract *pFaultListExtract, int num) {
    const size_t totalPatterns = pPatternCollector->patternVector_.size();
    const int totalThreads = 32; // 假设你打算将任务分配给2个不同的线程
    const size_t patternsPerThread = (totalPatterns + totalThreads - 1) / totalThreads; // 确保向上取整以覆盖所有patterns
    
    const size_t startIdx = num * patternsPerThread;
    size_t endIdx = startIdx + patternsPerThread;
    if (endIdx > totalPatterns) {
        endIdx = totalPatterns;
    }
    ...
    // Simulate only a subset of patterns for all faults.
    for (size_t i = startIdx; i < endIdx; ++i) {
        const Pattern &pattern = pPatternCollector->patternVector_[i];
